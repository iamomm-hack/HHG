          ...
          <AnimatePresence mode="wait" initial={false}>
            <motion.div key={step} className="step-content" initial={reduce ? false : { opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={reduce ? {} : { opacity: 0, x: -10 }} transition={{ duration: .18 }}>
              ...
              {step === 2 && <>
                ...
                {teamSize > 1 && <div className="member-name-section"><span>TEAM MEMBER DETAILS</span><div className={`member-name-grid ${teamSize === 3 ? "three-members" : ""}`}>{Array.from({ length: teamSize }, (_, index) => <div className="member-identity-fields" key={index}><strong className="member-field-title">MEMBER {index + 1}</strong><label className="field"><span>NAME <b>{memberNames[index].length}/26</b></span><input value={memberNames[index]} maxLength={26} placeholder={`Member ${index + 1} name`} onChange={(event) => setMemberName(index, event.target.value)} /></label><label className="field"><span>X TAG <small>OPTIONAL</small></span><input value={memberXUsernames[index]} maxLength={39} placeholder="X username" autoComplete="off" onChange={(event) => setMemberXUsername(index, event.target.value)} /></label></div>)}</div><small className="shared-role-note">All member ROLE lines use the Primary Role selected below.</small></div>}
